file_names = list.files(path = "C:\\Users\\anthony\\Desktop\\my_project\\2016_10_29\\modified_data", pattern = NULL, all.files = FALSE, full.names = FALSE, recursive = FALSE, ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE)

for(rds_file in file_names){

	my_path = paste0("C:\\Users\\anthony\\Desktop\\my_project\\2016_10_29\\modified_data\\",rds_file)
	air_data <- readRDS(my_path)
	my_path = paste0("C:\\Users\\anthony\\Desktop\\my_project\\2016_12_12\\Table_of_ChangHua\\",substr(rds_file,1,10), ".rds")
	eight_direct_dist_taipei <- readRDS(my_path)
	...
	air_data<-subset(air_data, air_data$GPS_latitude<24.43)
	air_data<-subset(air_data, air_data$GPS_latitude>24.00)
	air_data<-subset(air_data, air_data$GPS_longtitude<121.44)
	air_data<-subset(air_data, air_data$GPS_longtitude>120.45)
	...
	new_path = paste0("C:\\Users\\anthony\\Desktop\\my_project\\2016_12_12\\Table_of_ChangHua_Prob\\",substr(rds_file,1,10), ".rds")
	saveRDS(eight_dir_prob, file = new_path)
}